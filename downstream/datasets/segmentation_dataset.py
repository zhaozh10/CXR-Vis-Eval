...
from downstream.datasets.utils import resize_img
...
class SIIMSegDatasetV0(BaseImageDataset):
    def __init__(self, split="train", transform=None,
                 data_pct=0.01, phase="segmentation", imsize=1024):
        ...
        if split == "train":
            ...
            # with open(PNEUMOTHORAX_TRAIN_CSV, newline='') as csvfile:
...
# class SIIMSegDataset(BaseImageDataset):
#     def __init__(self, split="train", transform=None,
#                  data_pct=0.01, phase="segmentation", imsize=1024):
#         super().__init__(split, transform)
class SIIMSegDataset(BaseImageDataset):
    def __init__(self,  split="train", data_pct=0.01, imsize=512,transform=None):
        super().__init__(split, transform)
        self.imsize=imsize
        self.stats = json.load(open(PNEUMOTHORAX_STAT))[split]['scan']
        self.seg_transform=self.get_transforms()
        #read in csv file
        if split == "train":
            self.df = pd.read_csv(PNEUMOTHORAX_TRAIN_CSV)
            ...
            self.df = pd.read_csv(PNEUMOTHORAX_VALID_CSV)
            # with open(PNEUMOTHORAX_VALID_CSV, newline='') as csvfile:
            #     reader = csv.reader(csvfile, delimiter=',')
            #     self.samples = list(islice(reader, 1, None))
        else:
            self.df = pd.read_csv(PNEUMOTHORAX_TEST_CSV)
            # with open(PNEUMOTHORAX_TEST_CSV, newline='') as csvfile:
            #     reader = csv.reader(csvfile, delimiter=',')
            #     self.samples = list(islice(reader, 1, None))
    ...
    def get_transforms(self):
        ...
        list_transforms.extend(
            [
                Resize(self.imsize, self.imsize),
                Normalize(mean=self.stats['pixel_mean'], std=self.stats['pixel_std'], p=1),
                ToTensorV2(),
            ]
        )
    ...
    def __getitem__(self, index):
        ...
        mask=Image.open(os.path.join(str(PNEUMOTHORAX_MASK_DIR),f'{imgid}.png'))
        # mask=cv2.imread(os.path.join(str(PNEUMOTHORAX_MASK_DIR),f'{imgid}.png'))
        x=np.asarray(x)
        ...
        # segmentation_masks = {
        #     mask_name: PIL.Image.open(mask_path) for mask_name, mask_path in self._get_mask_paths(imgid).items()
        # }
        return x,y
        # return {
        #     "id": index,
